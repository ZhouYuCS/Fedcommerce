...
class DatasetWrapper(Dataset[T_co], metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.base_size=self.base_ratio
        self.incremental_size = self.num_classes -      self.base_size
    ...
    def get_divideed_indices(self,ratio,shuffle_seed):
        divided_indices=[[ ]for i in range(self.divide_number)]
        
        for class_indices in self.class_indices[self.base_size:]:
            start=0
            max_indice=len(class_indices)
            straw=copy.deepcopy(class_indices)
            Random(shuffle_seed).shuffle(straw)
            for do_index in range(self.divide_number):
                step=int(max_indice*ratio[do_index])
                divided_indices[do_index]+=straw[start:min(max_indice,start+step)]
                start+=step
